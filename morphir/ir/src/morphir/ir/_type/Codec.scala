...
object Codec {
  ...
  implicit def encodeConstructorArgs[A](
    ...
    morphir.sdk.list.Codec.encodeList(
      (
        (constructorArgs: (morphir.ir.Name.Name, morphir.ir.Type.Type[A])) =>
          io.circe.Json.arr(
        ...
        definition match {
          case morphir.ir.Type.CustomTypeDefinition(arg1, arg2) =>
            io.circe.Json.arr(
              ...
              morphir.ir.accesscontrolled.Codec
                .encodeAccessControlled(morphir.ir._type.Codec.encodeConstructors(encodeA))(arg2)
            )
    ...
    (
      (c: io.circe.HCursor) =>
        for {
          ...
          arg2 <- morphir.ir._type.Codec.decodeConstructorArgs(decodeA)(c)
        } yield (arg1, arg2)
    )
                  ...
                  case "CustomTypeDefinition" =>
                    for {
                      ...
                      arg2 <- c.downN(2)
                                .as(
                                  morphir.ir.accesscontrolled.Codec
                                    .decodeAccessControlled(morphir.ir._type.Codec.decodeConstructors(decodeA))
                                )
                ...
                tag match {
                  case "ExtensibleRecord" =>
                    for {
                      ...
                      arg3 <-
                        c.downN(3).as(morphir.sdk.list.Codec.decodeList(morphir.ir._type.Codec.decodeField(decodeA)))
                  ...
                  case "Record" =>
                    for {
                      arg1 <- c.downN(1).as(decodeA)
                      arg2 <-
                        c.downN(2).as(morphir.sdk.list.Codec.decodeList(morphir.ir._type.Codec.decodeField(decodeA)))
                  ...
                  case "Reference" =>
                    for {
                      ...
                      arg3 <-
                        c.downN(3).as(morphir.sdk.list.Codec.decodeList(morphir.ir._type.Codec.decodeType(decodeA)))
                  ...
                  case "Tuple" =>
                    for {
                      arg1 <- c.downN(1).as(decodeA)
                      arg2 <-
                        c.downN(2).as(morphir.sdk.list.Codec.decodeList(morphir.ir._type.Codec.decodeType(decodeA)))
